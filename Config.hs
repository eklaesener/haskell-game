...
notFoundStr opt = "Warning!\nThe \"" ++ opt
    ++ "\" option is unavailable, defaulting to "
...
initFullDefaults = do
    putStrLn $ "Warning!\nThe config file \"settings.cfg\" "
        ++ "could not be found or read. Using default values" ++ pressEnterStr
    _ <- getLine
    return fullDefaults
...
initGetMap = do
    ...
    case rv1 of
        Left _ -> case rv2 of
            Left _ -> do
                putStrLn $ "Warning!\nThe map size options are unavailable, "
                    ++ "defaulting to " ++ show def ++ pressEnterStr
                _ <- getLine
                return def
            Right cols -> do
...
initGetRoom = do
    ...
    case rv1 of
        Left _ -> case rv2 of
            Left _ -> do
                putStrLn $ "Warning!\nThe room size options are unavailable, "
                    ++ "defaulting to " ++ show def ++ pressEnterStr
                _ <- getLine
                return def
            Right cols -> do
...
initGetStartHP = do
    ...
    case rv of
        Left _ -> do
            putStrLn $ notFoundStr "startinghealthpoints"
                ++ show def ++ pressEnterStr
            _ <- getLine
            return def
        Right hp -> return hp
  where
...
initGetEnemies = do
    ...
    case rv1 of
        Left _ -> case rv2 of
            Left _ -> do
                putStrLn $ "Warning!\nThe \"minenemies\" and \"maxenemies\" "
                    ++ "options are unavailable, defaulting to "
                    ++ show def ++ pressEnterStr
                _ <- getLine
                return def
            Right n -> do
